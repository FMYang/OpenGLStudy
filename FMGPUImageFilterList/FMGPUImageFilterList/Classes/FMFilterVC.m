...
@interface FMFilterVC () <GPUImageVideoCameraDelegate> {
...
@property (nonatomic) UISlider *filterSettingsSlider;
...
- (instancetype)initWithFilterType:(GPUImageShowcaseFilterType)type {
    if(self = [super init]) {
        filterType = type;
    }
    return self;
...
- (void)viewDidLoad {
    ...
    _filterSettingsSlider = [[UISlider alloc] init];
    _filterSettingsSlider.frame = CGRectMake(0, 0, 300, 30);
    _filterSettingsSlider.center = CGPointMake(self.view.bounds.size.width * 0.5, self.view.bounds.size.height - 60);
    [_filterSettingsSlider addTarget:self action:@selector(updateFilterFromSlider:) forControlEvents:UIControlEventValueChanged];
    [filterView addSubview:_filterSettingsSlider];
...
//        }; break;
    ...
    if (filterType == GPUIMAGE_FILECONFIG)
    {
        ...
        pipeline = [[GPUImageFilterPipeline alloc] initWithConfigurationFile:[[NSBundle mainBundle] URLForResource:@"SampleConfiguration" withExtension:@"plist"] input:videoCamera output:filterView];
...
//        }
        else if (filterType == GPUIMAGE_MOTIONDETECTOR)
        {
            ...
            __unsafe_unretained FMFilterVC * weakSelf = self;
