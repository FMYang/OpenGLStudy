...
@property (nonatomic, assign) CMTime nextFrameTimestamp;
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer ofMediaType:(NSString *)mediaType
{
	...
	dispatch_async( _writingQueue, ^{
		...
		@autoreleasepool
		{
            ...
            if (!CMTIME_IS_VALID(self.startWriteTime) ) {
                if ( ! _haveStartedSession ) {
                    if (mediaType ==AVMediaTypeVideo) {
                        ...
                        [_assetWriter startSessionAtSourceTime:self.startWriteTime];
                        self.nextFrameTimestamp = self.startWriteTime;
			...
			if ( input.readyForMoreMediaData && mediaType == AVMediaTypeVideo)
			{
//				BOOL success = [input appendSampleBuffer:sampleBuffer];
                CVPixelBufferRef pixelBuf = CMSampleBufferGetImageBuffer(sampleBuffer);
                [self writePixelBuffer:pixelBuf presentationTime:self.nextFrameTimestamp input:_videoInput];
                self.nextFrameTimestamp = CMTimeAdd(self.nextFrameTimestamp, CMTimeMake((1.0 / 30) * 10000, 10000)); // >46就开始掉帧了
                [self movieDuration];
...
- (void)writePixelBuffer:(CVPixelBufferRef)pixelBufferRef presentationTime:(CMTime)presentationTime input:(AVAssetWriterInput *)input {
    @autoreleasepool {
        NSLog(@"writePixelBuffer");
//        BOOL success = [_pixelBufferAdaptor appendPixelBuffer:pixelBufferRef withPresentationTime:presentationTime];
...
- (BOOL)setupAssetWriterVideoInputWithSourceFormatDescription:(CMFormatDescriptionRef)videoFormatDescription transform:(CGAffineTransform)transform settings:(NSDictionary *)videoSettings error:(NSError **)errorOut
{
	...
	{
//        NSMutableDictionary *commentedCompressionProperties = [[videoSettings objectForKey:AVVideoCompressionPropertiesKey] mutableCopy];
//        commentedCompressionProperties[@"ExpectedFrameRate"] = @(30);
//        commentedCompressionProperties[AVVideoAllowFrameReorderingKey] = @(NO);
//        NSMutableDictionary *newVideoSettings = [videoSettings mutableCopy];
//        [newVideoSettings setValue:commentedCompressionProperties forKey:AVVideoCompressionPropertiesKey];
        ...
        _pixelBufferAdaptor = [[AVAssetWriterInputPixelBufferAdaptor alloc] initWithAssetWriterInput:_videoInput sourcePixelBufferAttributes:attributes];
